...
<body>
    ...
    <script>
        ...
        tableBody.addEventListener("click",e =>{
            const botton = e.target.closest('button');
            const btnEliminar = document.querySelector('.confirm');
            btnEliminar.addEventListener('click',()=>{
                if(botton){
                    const fila = botton.closest('tr');
                    const id = fila.getAttribute('data-id');
                    const action = botton.getAttribute('data-action');

                    if(action == 'delete'){
                        fila.remove();

                        fetch(`../../../app/trigger/person1.php?action=DELETE&id=${id}`)
                            .then(res => res.json())
                            .then(console.log)
    ...
    <!-- Si da pero algunas cosas no da -->
